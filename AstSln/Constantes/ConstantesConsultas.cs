...
namespace Constantes
{
    public class ConstantesConsultas
    {
        ...
        public const string QUERY_AGRUPAR_APARICIONES_DESP_ACTUAL = "SELECT count(*) AS CountRank, {0} AS ClaveNum FROM {1} WHERE fecha IN (SELECT fecha+1 FROM {1} WHERE {0} = {2} AND fecha < to_date('{3}','dd/MM/yyyy')) GROUP BY {0} ORDER BY 1";
